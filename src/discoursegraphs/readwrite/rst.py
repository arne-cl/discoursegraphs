...
class RSTLispDocumentGraph(DiscourseDocumentGraph):
    ...
    def __init__(self, dis_filepath, name=None, namespace='rst',
        ...
        self.tokens = []
        self.rst_tree = self.parse_rstfile_sexpdata(dis_filepath)
        self.parse_rst_tree(self.rst_tree)

    def parse_rstfile_sexpdata(self, rst_filepath):
        with open(rst_filepath) as rstfile:
            #~ try:
            return sexpdata.load(rstfile)
            #~ except sexpdata.ExpectClosingBracket as e:
                #~ raise sexpdata.ExpectClosingBracket(u"{}\n{}\n\n".format(rst_filepath, e))
            #~ except sexpdata.ExpectNothing as e:
                #~ error_msg = e.args[0][:100] # complete msg would contain the whole document
                #~ raise sexpdata.ExpectNothing(u"{}\n{}...\n\n".format(rst_filepath, e.args[0][:100]))
            #~ except AssertionError as e:
                #~ raise AssertionError(u"{}\n{}\n\n".format(rst_filepath, traceback.format_exc()))
