...
class MMAXProject(object):
    ...
    def __init__(self, project_path):
        self.paths, self.annotations, self.stylesheet = self._parse_common_paths_file(project_path)
    ...
    def _parse_common_paths_file(self, project_path):
        ...
        -------
        paths : dict
            maps from MMAX file types (str, e.g. 'basedata' or 'markable') to
            the relative path (str) containing files of this type
        annotations : dict
            maps from MMAX annotation level names (str, e.g. 'sentence', 'primmark')
            to a dict of features. The features are: 'schemefile' (maps to a file),
            'customization_file' (ditto) and 'file_extension' (maps to the file
            name ending used for all annotations files of this level)
        stylefile : str
        ...
        paths = {}
        path_vars = ['basedata', 'scheme', 'style', 'style', 'customization', 'markable']
        ...
        for level in tree.iterfind('//level'):
            annotations[level.attrib['name']] = {'schemefile': level.attrib['schemefile'], 
...
class MMAXDocumentGraph(MultiDiGraph):
    ...
    def __init__(self, mmax_rootdir, mmax_base_file):
        ...
        for layer_name in mmax_project.annotations:
            ...
            annotation_file = os.path.join(mmax_rootdir, 
    ...
    def get_file_id(self, mmax_base_file):
        ...
        """
        return os.path.basename(mmax_base_file)[:-5] # removes '.mmax' from filename
    ...
    def add_token_layer(self, words_file):
        ...
        for word in etree.parse(words_file).iterfind('//word'):
            self.add_node(word.attrib['id'], token=word.text, layers={'mmax', 'mmax:token'})
            self.add_edge('mmax:root_node', word.attrib['id'], layers={'mmax', 'mmax:token'})
            
    def add_annotation_layer(self, annotation_file):
        """
        """
        assert os.path.isfile(annotation_file),             "Annotation doesn't exist: {}".format(annotation_file)
        pass
    ...
    def get_annotation_type():
        """
        TODO: watch out for cross-layer annotations, e.g. 'anaphor_antecedent="secmark:markable_18"'
        """
        pass
...
def span2tokens(span_string):
    ...
    for span in spans:
        ...
        if len(span_elements) == 1:
            ...
            end_id = int(end[5:])
            tokens.extend(['word_'+str(token_id) for token_id in range(start_id, end_id+1)])
        else:
...
if __name__ == "__main__":
    MMAX_ROOTDIR = os.path.expanduser("~/repos/pcc-annis-merged/maz176/coreference")
    COMMON_PATHS_FILE = os.path.expanduser("~/repos/pcc-annis-merged/maz176/coreference/common_paths.xml")
    BASEDATA_DIR = os.path.join(MMAX_ROOTDIR, MMAXProject(MMAX_ROOTDIR).paths['basedata'])
